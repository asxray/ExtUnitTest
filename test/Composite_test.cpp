...
class brick : public dp::Component {
 public:
  VISITABLE();
  brick() : n(1) {}
  virtual ~brick() {}
...
class wall : public dp::Composite {
 public:
  VISITABLECOMPOSITE();
  wall() : n(3) {}
  virtual ~wall() {}
...
class Insect : public dp::Visitor<brick>, public dp::Visitor<wall> {
 public:
  ...
  virtual ~Insect() {}
  virtual void Visit(brick* const aPlace) {
    ++brickcount;
...
BOOST_AUTO_TEST_CASE(composite) {
  ...
  //    redbrick->Add(blackbrick);  // won't compile. component should not add
  // another one.
